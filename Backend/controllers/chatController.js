...
export const createChatTitle = async (req, res) => {
  try {
    ...
    if (existing)
      return res
        .status(400)
        .json({ success: false, message: "Title already exists for this user" });

    const chat = await Chat.create({ userId, title, messages: [] });
    res
      .status(201)
      .json({ success: true, message: "Chat created", chatId: chat._id });
  } catch (err) {
...
export const getTitlesByUser = async (req, res) => {
  try {
    const { id: userId } = req.user;
